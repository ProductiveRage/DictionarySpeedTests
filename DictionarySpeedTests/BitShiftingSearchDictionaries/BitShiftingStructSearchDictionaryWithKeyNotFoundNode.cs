...
{
    public class BitShiftingStructSearchDictionaryWithKeyNotFoundNode<TKey, TValue> : ILookup<TKey, TValue>
    {
        private Node[] _nodes;
        private KeyValuePair<TKey, TValue>[] _values;
        private IEqualityComparer<TKey> _keyComparer;
        public BitShiftingStructSearchDictionaryWithKeyNotFoundNode(Dictionary<TKey, TValue> data)
        {
            if (data == null)
            ...
            // Inject a Key-Not-Found node which all paths for invalid Keys will fall into, this saves us from having to check the EvenNodeIndex and OddNodeIndex
            // values for -1 in the Get method when the Key hash gets processed
            var keyNotFoundNodeIndex = nodes.Count;
            nodes.Add(new Node() { EvenNodeIndex = keyNotFoundNodeIndex, OddNodeIndex = keyNotFoundNodeIndex, ValueIndices = null });
            for (var index = 0; index < nodes.Count; index++)
            {
                if (nodes[index].EvenNodeIndex == -1)
                    nodes[index] = SetEvenNodeIndex(nodes[index], keyNotFoundNodeIndex);
                if (nodes[index].OddNodeIndex == -1)
                    nodes[index] = SetOddNodeIndex(nodes[index], keyNotFoundNodeIndex);
            }
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
            ...
            while (hash > 0)
            {
                // Note: No need to ever check whether nodeIndex gets set to -1 here due to the KeyNotFound Node injected into the data by the constructor
                var isEven = (hash & 1) == 0;
